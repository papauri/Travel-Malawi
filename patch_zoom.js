...
code = code.replace(/map\.fitBounds\(bounds, \{ padding: \[60, 60\], maxZoom: 15 \}\);/g, "map.fitBounds(bounds, { padding: [140, 140], maxZoom: 13 });");
code = code.replace(/map\.flyTo\(\[lodge\.coordinates\.lat, lodge\.coordinates\.lng\], Math\.max\(map\.getZoom\(\), 15\)/g, "map.flyTo([lodge.coordinates.lat, lodge.coordinates.lng], Math.max(map.getZoom(), 12)");
